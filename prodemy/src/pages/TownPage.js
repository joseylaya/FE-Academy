...
import UpdateUser from '../middleware/towns/townUpdate';
import AddUser , { LoaderTable } from '../middleware/towns/townAdd';
...
export default function DataTable() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8000/fetchTown');
  ...
  const columns = [
    ...
    { field: 'latitude', headerName: 'Latitide', width: 150 },
    ...
    // { field: 'Teamname', headerName: 'Team', width: 110 },
    {
      field: 'featured',
      headerName: 'Featured',
      width: 80,
      renderCell: (params) => (
        <>
          {params.row.featured === 1? <Chip label="Yes" color="primary" size="small"/>  : <Chip label="No" color="primary" variant="outlined" size="small"/>}
        </>
      ),
    },
    // {
    ...
    //   renderCell: (params) => (
    //     <div>
    //       {params.row.Role === 0? 'Admin' : 'Student'}
    //     </div>
    //   ),
    // },
    {
