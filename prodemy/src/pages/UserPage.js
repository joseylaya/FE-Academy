...
import { Avatar, Container, Typography, Stack, Card,  IconButton, Popover, MenuItem, Chip } from '@mui/material';
...
import UpdateUser from '../middleware/user/userUpdate';
import AddUser , { LoaderTable } from '../middleware/user/userAdd';
...
export default function DataTable() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8000/fetchUser');
  ...
  const handleSearch = (event) => {
    ...
    const filteredData = rows.filter(
      (row) =>
        ...
        row.about_me.toLowerCase().includes(keyword.toLowerCase())||
  ...
  const handleUpdate = async () => {
    try {
      ...
      await fetch(`http://localhost:8000/updateUser/${editedRow.id}`, {
        method: 'PUT',
        headers: {
  ...
  const columns = [
    ...
    {field: 'id', headerName: 'ID', width: 20},
    {
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ marginLeft: '8px' ,fontWeight: '600'}}>{params.row.name}</span>
        </div>
      ),
    },
    { field: 'email', headerName: 'Email', width: 250 },
    { field: 'about_me', headerName: 'About Me', width: 250 },
    {
      field: 'created_at',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <>
          {params.row.Status === 0? <Chip label="inactive" color="primary" variant="outlined" size="small"/> : <Chip label="active" color="primary" size="small"/> }
        </>
      ),
    },
    {
  ...
  return (
    ...
    <Popover
      ...
      >
        <MenuItem onClick={() => handleOpenModal(menuRowId)}>
